...
def read_logs(ChildID: int) -> str:
    ...
    """
    try:
        # Construct path using the now correctly set DATA_DIR
...
def append_logs(ChildID: int, log_entry: dict) -> str:
    """
    Appends a new log entry for a given child ID.
    Args:
        ChildID: The unique integer identifier for the child to append logs for.
        log_entry: A dictionary representing the new log entry.
    Returns:
    ...
    try:
        ...
        return json.dumps({"status": "success", "message": f"Log entry added for child ID {ChildID} at path: {logs_fp}."})
    except Exception as e:
...
child_log_agent = Agent(
    ...
    model=AGENT_MODEL,
    description="Manages and accesses log information for specific children.",
    instruction=(
        ...
        "You will need the child's ID and the details of the log entry. "
        ...
        "The Child ID MUST be a numerical value (e.g., '123', '456'). Do not use text like 'child-one'."
        "When reading logs, provide the content of the logs to the user."
        "When appending logs, confirm that the log was added."
    ),
...
async def main():
    ...
    # Test case 1: Read logs for Child ID 1 (should exist)
    await call_agent_async("Read the logs for Child ID 1", runner, USER_ID, SESSION_ID)

    # Test case 2: Read logs for Child ID 2 (should NOT exist, testing error handling)
    # This should now explicitly say "Child profile not found"
    await call_agent_async("What's in the logs for Child ID 2?", runner, USER_ID, SESSION_ID)

    # Test case 3: Append a log entry for Child ID 1
    await call_agent_async("Add an entry for Child ID 1: {'activity': 'coloring', 'outcome': 'created a masterpiece'}", runner, USER_ID, SESSION_ID)

    # Test case 4: Append a log entry for Child ID 2 (should now return error as profile is not found)
    await call_agent_async("Add a log for Child ID 2: {'event': 'first steps', 'date': '2025-06-25'}", runner, USER_ID, SESSION_ID)

    # Test case 5: Read logs for Child ID 2 again (should still show "not found" unless you manually create child_002)
    await call_agent_async("Read the logs for Child ID 2", runner, USER_ID, SESSION_ID)
